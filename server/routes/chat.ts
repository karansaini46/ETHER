...
import type { ChatMessage, NavCommand } from '../../src/types/navigator'
...
chatRouter.post('/api/chat', async (request, response) => {
  const { message, graph, history } = request.body as {
    message: string
    graph: GraphData
    history: ChatMessage[]
  }
  ...
  if (!apiKey) {
    response.status(200).json({
      role: 'assistant',
      content: 'Gemini API key is not configured on the server. Please add VITE_GEMINI_API_KEY to your .env file.',
      timestamp: Date.now(),
    })
    return
  }
...
---COMMAND---
{
  ...
  try {
    ...
    response.status(200).json({
      role: 'assistant',
      content: isRateLimit
        ? 'Rate limited or quota exceeded on free tier. Please wait a moment or check your API key settings.'
